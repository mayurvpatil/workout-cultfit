...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0d12] text-zinc-100 font-sans selection:bg-lime-500 selection:text-black relative overflow-x-hidden">
      ...
      {/* Header */}
      <header className="p-4 sm:p-5 border-b border-zinc-800/70 bg-[#0a0d12]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-3xl sm:text-4xl font-black uppercase italic tracking-tight flex items-center gap-2 text-white">
              CultFit <span className="text-lime-400">Planner</span>
            </h1>
            <p className="text-zinc-400 text-xs sm:text-sm mt-1 uppercase tracking-[0.22em] font-semibold flex items-center gap-3">
              Push 
          ...
          <div className="hidden sm:block">
             <div className="px-3 py-1 border border-lime-500/30 bg-lime-500/10 rounded-full text-xs font-bold tracking-widest text-lime-300 uppercase flex items-center gap-2">
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-5 space-y-12 py-8">
        {/* Weekly Plan */}
        <section>
            ...
            {weeklyPlan.map(day => {
              ...
              return (
                <button
                  ...
                  className={`p-4 border transition-all duration-300 flex flex-col justify-center relative overflow-hidden group text-left rounded-xl ${
                    isSelected
                ...
                >
                  ...
                  <div className={`font-black text-lg uppercase tracking-wide relative z-10 transition-colors ${isSelected ? 'text-lime-400' : 'text-zinc-100 group-hover:text-lime-400'}`}>{day.type}</div>
                </button>
              );
            })}
          </div>
        ...
        {/* Picker */}
        <section className="bg-gradient-to-br from-zinc-900/95 to-zinc-950 p-6 sm:p-7 border border-zinc-700/70 rounded-2xl relative overflow-hidden shadow-[0_16px_40px_rgba(0,0,0,0.35)]">
          ...
          <div className="flex flex-col gap-1.5 mb-6 relative z-10">
            <h2 className="text-3xl sm:text-4xl font-black uppercase italic tracking-tight text-white">Quick Start</h2>
            <p className="text-zinc-400 text-xs sm:text-sm font-medium uppercase tracking-[0.2em]">Get random workout video</p>
          </div>
          
          <div className="flex flex-col sm:flex-row gap-3 mb-6 relative z-10">
            <select value={pickerType} onChange={e => setPickerType(e.target.value)} className="px-4 py-3 bg-zinc-950/90 border border-zinc-700 font-bold uppercase tracking-wider shadow-sm outline-none focus:border-lime-500 rounded-lg text-zinc-200">
              {workoutGroups.map(group => (
                <option key={group.key} value={group.key}>{group.label}</option>
              ))}
            </select>
            <select value={pickerDuration} onChange={e => setPickerDuration(e.target.value)} className="px-4 py-3 bg-zinc-950/90 border border-zinc-700 font-bold uppercase tracking-wider shadow-sm outline-none focus:border-lime-500 rounded-lg text-zinc-200">
              <option value="long">45–60 Minutes</option>
              <option value="short">Under 40 Minutes</option>
            </select>
            <button onClick={handleGenerate} className="px-6 py-3 bg-lime-500 text-black font-black uppercase tracking-widest hover:bg-lime-400 rounded-lg transition transform hover:-translate-y-0.5 active:translate-y-0 shadow-[0_8px_20px_rgba(163,230,53,0.25)]">
              Get Video
            </button>
          </div>
          ...
          {randomVideo && (
            <div className="p-[1px] max-w-xl bg-gradient-to-br from-lime-400 via-emerald-500 to-cyan-500 relative z-10 animate-fade-in-up rounded-xl">
              <div className="bg-zinc-950/95 p-5 sm:p-6 flex flex-col rounded-xl">
                ...
                <div className="flex items-center gap-2 text-sm text-zinc-400 font-bold uppercase tracking-wide mb-4">
        ...
        <section>
          ...
          <div className="flex border-b border-zinc-800/70 mb-6 overflow-x-auto">
            {workoutGroups.map(group => (
              <button 
                key={group.key} 
                onClick={() => setTabType(group.key)} 
                className={`px-6 py-3 font-black uppercase tracking-widest transition-all whitespace-nowrap rounded-t-lg ${
                  tabType === group.key 
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xl font-black uppercase tracking-widest mb-4 flex items-center gap-2 text-zinc-300">
      ...
      <footer className="py-8 border-t border-zinc-800/70 bg-[#0a0d12]/90 mt-12">
        <div className="max-w-6xl mx-auto flex flex-col items-center justify-center p-4 gap-3">
          <div className="text-2xl font-black uppercase italic tracking-tight text-zinc-600">CultFit Planner</div>
